...
def draw_bbox(image, bboxes, depth_frame, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):   
    ...
    # with open("./all_boxes.txt", 'w+') as f:
    #     f.close()
    ...
    for i, bbox in enumerate(bboxes):
        ...
        cv2.circle(image, (locX, locY), 4, (0, 0, 255))
        #distance = depth_frame[locY, locX]
        #cv2.putText(image, "{}m".format(distance/1000), (locX, locY - 20), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 0), 2)
...
def postprocess_boxes(pred_bbox, original_image, input_size, score_threshold):
    ...
    score_mask = max(scores)
    mask = np.logical_and(scale_mask, score_mask)
    coors, scores, classes = pred_coor[mask], scores[mask], classes[mask]

    return np.concatenate([coors, scores[:, np.newaxis], classes[:, np.newaxis]], axis=-1), scale_mask


def det_obj_est_dis(Yolo, image_path, depth_frame, input_size=416, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    bboxes, scoreMAX = postprocess_boxes(pred_bbox, original_image, input_size, score_threshold)
    bboxes = nms(bboxes, iou_threshold, method='nms')

    image, X, Y = draw_bbox(original_image, bboxes, depth_frame, CLASSES=CLASSES, rectangle_colors=rectangle_colors)
        
    return image, X, Y, scoreMAX
